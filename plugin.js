...
import {
  PlanetType,
  SpaceType,
} from "https://cdn.skypack.dev/@darkforest_eth/types"; //@ts-ignore
...
import {
  ...
  useLayoutEffect,
  useState,
} from "https://unpkg.com/htm/preact/standalone.module.js"; // User Configurable Options
...
let STAGGER_S = 300; // Over what number of seconds will all repeat attacks happen once?
// UI controls

let TRIGGER_PENDING = 5000; // milliseconds to sleep if a attack is triggered, it's expected
// to be the time your pc calculate the snarks
...
function unconfirmedDepartures(planet) {
  return (
    planet.transactions
      ?.getTransactions(isUnconfirmedMoveTx) //@ts-ignore
...
class Repeater {
  constructor() {
    ...
    if (typeof window.__CORELOOP__ == "undefined") {
      ...
      console.log("KILLING PREVIOUS INTERVALS"); //@ts-ignore
    ...
    this.loadAttacks();
    this.intervalId = window.setInterval(this.coreLoop.bind(this), 1000); //@ts-ignore
    localStorage.setItem("ra-executing", false);
  ...
  loadAttacks() {
    const attacksJSON = localStorage.getItem(
      `repeatAttacks-${this.account}-${this.configHash}`
    ); // @ts-ignore
  ...
  addAttack() {
    const attack = { ...this.currentAttack }; // Make sure source and target set

    if (!attack.sourceId || !attack.targetId) return; // Make sure remaining energy is less than trigger energy

    attack.pcRemain =
      attack.pcTrigger <= attack.pcRemain
        ? parseInt(attack.pcTrigger / 2)
        : attack.pcRemain; // Filter out existing attacks from this source, then add new attack at top
  ...
  async coreLoop() {
    ...
    if (localStorage.getItem("ra-executing") == "true") return;
    console.log("set to executing to true");
    localStorage.setItem("ra-executing", true);
    for (const a of this.attacks) {
      await ExecuteAttack(a);
    }
    localStorage.setItem("ra-executing", false);
    console.log("set to executing to false");
  }
}
...
const ExecuteAttack = async ({
  ...
  if (!srcPlanet) return;
  if (!active) return; // Needs updated check getUnconfirmedDepartingForces
  ...
  if (FUZZY_ENERGY > TRIGGER_AMOUNT) {
    // if trigger successfully, sleep
    await sleep(TRIGGER_PENDING);
